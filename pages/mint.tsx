...
const Mint: NextPageWithLayout = () => {
  ...
  const handMint = async (values: any) => {
    ...
    if (address.includes("addr_test")) {
      address_cost =
        `${process.env.NEXT_PUBLIC_ADDRESS_TEST}` || ADDRESS_TESTNET;
    }
    console.log("address_cost----", royalty, address_cost);

    if (royalty) {
      tx.sendLovelace(address_cost, `${FEE}000000`);
          ...
          <Col xs={24} sm={12}>
            ...
            <Card title="Mint NFTs/Token">
              ...
              <Space style={{ margin: "10px 0" }}>
                <Text>
                  Professional ({FEE} ada):{" "}
                  <Tooltip title="Pro will remove royalty">
                    <InfoCircleOutlined />
                  </Tooltip>{" "}
                </Text>
              ...
              <Row gutter={[16, 20]} style={{ marginTop: "20px" }}>
                <Col span={24}>
                  <Button block onClick={handAddProperty}>
                    Add Property <PlusOutlined />
                  </Button>
                </Col>
                <Col span={12}>
                  <Button block onClick={handReset}>
                    Reset
                  </Button>
                </Col>
                <Col span={12}>
